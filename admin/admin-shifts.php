  ...
  <script>
      ...
      render() {
        ...
        let html = `
            <div class="jdp-header">
              <button class="jdp-nav-btn" data-nav="-1">&rarr;</button>
              <div>${new Intl.DateTimeFormat("fa-IR", { month: "long" }).format(
          firstG
        )} ${new Intl.DateTimeFormat("fa-IR", { year: "numeric" }).format(
          firstG
        )}</div>
              <button class="jdp-nav-btn" data-nav="1">&larr;</button>
            </div>
            <div class="jdp-grid">
              ${weekDays
        ...
        this.pop.querySelectorAll("[data-day]").forEach((cell) => {
          cell.addEventListener("click", (e) => {
            ...
            this.input.value = formatJalaliDisplay(this.jy, this.jm, d); // نمایش جلالی
            this.alt.value = formatISO(gDate); // مقدار ISO برای فیلتر
            this.hide();
    ...
    async function initializePage() {
      ...
      try {
        ...
        // راه‌اندازی تاریخ‌پیکرهای جلالی + مقادیر پیش‌فرض (امروز تا 7 روز بعد)
    ...
    function setupEventListeners() {
      // ورودی‌های جلالی روی blur/change هم فیلتر رو اعمال کنن
      document
    ...
    async function saveShiftUpdate() {
      ...
      try {
        const response = await fetch("/php/update-shift.php", {
          ...
          body: JSON.stringify({ expertId, date, status: newStatus }),
        });
    ...
    function renderTable(expertsToRender, datesToRender) {
      ...
      const uniqueShiftTimes = [
        ...new Set(allExperts.map((e) => e["shifts-time"]).filter(Boolean)),
      ].sort();
      ...
      if (uniqueShiftTimes.length > 0) {
        ...
        uniqueShiftTimes.forEach((shiftTime) => {
          ...
          datesToRender.forEach((date) => {
            tableHtml += `<td><span class="summary-count">${dailyOnDutyCounts[date][shiftTime] || 0
              }</span></td>`;
          });
          tableHtml += `</tr>`;
          ...
          datesToRender.forEach((date) => {
            tableHtml += `<td><span class="summary-count" style="color: #dc3545;">${dailyOffDutyCounts[date][shiftTime] || 0
              }</span></td>`;
          });
          tableHtml += `</tr>`;
          ...
          datesToRender.forEach((date) => {
            tableHtml += `<td><span class="summary-count" style="color: #ffc107; text-shadow: 1px 1px 1px #fff;">${dailyLeaveCounts[date][shiftTime] || 0
              }</span></td>`;
          });
          tableHtml += `</tr>`;
        });
      ...
      datesToRender.forEach((date) => {
        tableHtml += `<td><span class="summary-count">${totalOnDutyByDate[date] || 0
          }</span></td>`;
      });
      tableHtml += `</tr>`;
      ...
      datesToRender.forEach((date) => {
        tableHtml += `<td><span class="summary-count" style="color: #dc3545;">${totalOffDutyByDate[date] || 0
          }</span></td>`;
      });
      tableHtml += `</tr>`;
      ...
      datesToRender.forEach((date) => {
        tableHtml += `<td><span class="summary-count" style="color: #ffc107; text-shadow: 1px 1px 1px #fff;">${totalLeaveByDate[date] || 0
          }</span></td>`;
      });
      tableHtml += `</tr>`;
    ...
    function populateFilterControls() {
      ...
      // تاریخ‌های پیش‌فرض توسط DatePicker ست می‌شود (نیازی به ست مستقیم ورودی‌ها نیست)
    }

    function applyFilters() {
      // ❗ مهم: از فیلدهای مخفی ISO بخوان
      const startDate = document.getElementById("startDateAlt").value; // YYYY-MM-DD
      const endDate = document.getElementById("endDateAlt").value; // YYYY-MM-DD
